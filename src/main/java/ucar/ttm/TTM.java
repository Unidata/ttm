...
Another issues involves the character set. Since Java
...
static boolean issegmark(char c) {return testMark(c,SEGMARK);}
static boolean iscreate(char c) {return testMark(c,CREATE);}
static boolean ismark(char c) {return issegmark(c)||iscreate(c);}

static boolean iscontrol(char c) {return (c < ' ' || c == 127);}
static boolean iswhitespace(char c) {return (iscontrol(c) || c == ' ');}

static boolean isdec(char c) {return (c >= '0' && c <= '9');}

static boolean ishex(char c) {return (c >= '0' && c <= '9')
                                      || (c >= 'a' && c <= 'f')
                                      || (c >= 'A' && c <= 'F');}
...
public void setOutput(PrintWriter pw, boolean isstdout)
	{this.output = pw; this.isstdout = isstdout;}

public Reader getInput() {return this.rsinput;}
public void setInput(Reader r, boolean isstdin)
	{this.rsinput = r; this.isstdin = isstdin;}

public boolean isstdout() {return this.isstdout;}
public boolean isstdin() {return this.isstdin;}
...
// Test flags
public boolean testFlag(int flag) {return (flags & flag) != 0;}
...
exec(Active active, StringBuilder passive)
{
    ...
    if((flags & FLAG_EXIT) != 0) {popFrame(); return;}
...
call(Frame frame, String body)
{
	...
	char c = body.charAt(p);
	    ...
	    // construct the create mark text
            crcounter++;
	    String crmark = String.format("%d",crcounter);
...
main(String[] argv)
{
    ...
    try {if(!ttm.isstdin()) ttm.getInput().close();} catch(IOException ioe) {}
